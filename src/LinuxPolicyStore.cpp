...
LinuxPolicyStore::LinuxPolicyStore(const BrowserDefinition& b, PolicyScope s, const QString& overrideRoot)
	...
	if (root_.isEmpty())
		root_ = qEnvironmentVariable("QCC_LINUX_POLICY_ROOT");
	if (root_.isEmpty())
		root_ = browser_.linuxPolicyRoot;
}
...
PolicyState LinuxPolicyStore::load() {
	...
	if (o.contains("CACertificatesWithConstraints")) {
		if (!o["CACertificatesWithConstraints"].isArray())
			throw std::runtime_error("CACertificatesWithConstraints must be an array.");
...
void LinuxPolicyStore::save(const PolicyState& s) {
	...
	if (!QDir().mkpath(dir))
		throw std::runtime_error("Cannot create Chrome policy directory.");

	QJsonObject o;
	if (s.platformIntegrationEnabled.has_value())
		o["CAPlatformIntegrationEnabled"] = *s.platformIntegrationEnabled;
	if (!s.entries.isEmpty())
		o["CACertificatesWithConstraints"] = entriesToJson(s.entries);
	...
	if (!f.open(QIODevice::WriteOnly))
		throw std::runtime_error(("Cannot write " + filePath()).toStdString());
	f.write(QJsonDocument(o).toJson(QJsonDocument::Indented));
	if (!f.commit())
		throw std::runtime_error("Cannot commit Chrome policy file.");
}
