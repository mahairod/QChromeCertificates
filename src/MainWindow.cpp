...
QString MainWindow::tr(const QString& ru,const QString& en)const {
	return settings_.language=="ru"?ru:en;
}
void MainWindow::setupUi() {
	...
	top->addWidget(new QLabel(tr("Браузер:","Browser:")));
	browserBox_=new QComboBox;
	for(auto b:BrowserDefinition::all())browserBox_->addItem(b.name,b.id);
	browserBox_->setCurrentText(browser_.name);
	top->addWidget(browserBox_,1);
	top->addSpacing(15);
	top->addWidget(new QLabel(tr("Язык:","Language:")));
	...
	top->addSpacing(15);
	top->addWidget(new QLabel(tr("Политика:","Policy:")));
	scopeBox_=new QComboBox;
	scopeBox_->addItems({"Managed","Recommended"});
	...
	table_->setHorizontalHeaderLabels({"Certificate","SHA-1","Allowed domains","Valid until"});
	...
	add->setText(tr("Добавить сертификат…","Add cert…"));
	remove_->setText(tr("Удалить сертификат","Remove cert"));
	imp->setText(tr("Импорт…","Import…"));
	exp->setText(tr("Экспорт…","Export…"));
	...
	status_=new QLabel;
	apply_=new QPushButton(tr("Применить","Apply"));
	cancel_=new QPushButton(tr("Отмена","Cancel"));
...
void MainWindow::reload() {
	try {
		...
		if(!state_.entries.isEmpty())isolation_->setChecked(true);
		updating_=false;
		refreshTable();
		baseline_=signature();
		updateButtons();
		...
		updating_=false;
		QMessageBox::critical(this,tr("Ошибка","Error"),tr("Не удалось прочитать политики:\n\n","Could not read policies:\n\n")+e.what());
	}
}
void MainWindow::refreshTable() {
	...
	for(const auto& e:state_.entries) {
		try {
			...
			int r=table_->rowCount();
			table_->insertRow(r);
			table_->setItem(r,0,new QTableWidgetItem("Invalid certificate"));
		}
	}
...
void MainWindow::updateButtons() {
	...
	setWindowTitle("QChromeCertificates 3.0"+QString(d?" *":""));
	status_->setStyleSheet((machinePolicies_?"color:#b36b00;":""));
}
void MainWindow::browserChanged(int i) {
	if(updating_||i<0)return;
	if(dirty()) {
		QMessageBox::warning(this,tr("Есть изменения","Unapplied changes"),tr("Сначала примените или отмените изменения.","Apply or cancel the changes first."));
		updating_=true;
		browserBox_->setCurrentText(browser_.name);
		updating_=false;
		return;
...
void MainWindow::isolationChanged(bool on) {
	...
	if(!on&&!state_.entries.isEmpty()) {
		if(QMessageBox::question(this,tr("Удаление сертификатов","Remove certificates"),tr("Отключение изоляции удалит все добавленные сертификаты при применении изменений. Продолжить?","Turning off isolation will remove all added certificates when changes are applied. Continue?"))!=QMessageBox::Yes) {
			updating_=true;
...
void MainWindow::addCertificate() {
	QString p=QFileDialog::getOpenFileName(this,tr("Выберите корневой сертификат","Select a root certificate"), {},tr("Certificates (*.cer *.crt *.der);;All files (*)","Certificates (*.cer *.crt *.der);;All files (*)"));
	if(p.isEmpty())return;
	try {
		auto m=readCertificateFile(p);
		if(!m.isCA)throw std::runtime_error("The selected certificate is not a CA certificate.");
		for(const auto& e:state_.entries)if(readCertificate(QByteArray::fromBase64(e.certificateBase64.toLatin1())).sha1.compare(m.sha1,Qt::CaseInsensitive)==0) {
				QMessageBox::information(this,"Certificate",tr("Этот сертификат уже добавлен.","This certificate has already been added."));
				return;
			}
		...
		QMessageBox::critical(this,tr("Ошибка","Error"),tr("Не удалось добавить сертификат:\n\n","Could not add the certificate:\n\n")+e.what());
	}
}
void MainWindow::removeCertificate() {
	int r=table_->currentRow();
	if(r<0) {
		QMessageBox::information(this,"Certificate",tr("Выберите сертификат в списке.","Select a certificate in the list."));
		return;
	}
	auto m=readCertificate(QByteArray::fromBase64(state_.entries[r].certificateBase64.toLatin1()));
	if(QMessageBox::question(this,"Confirmation",tr("Удалить сертификат «","Remove certificate “")+m.commonName+tr("»?","”?"))==QMessageBox::Yes) {
...
void MainWindow::editDomains() {
	...
	if(!state_.entries[r].cidrs.isEmpty()) {
		QMessageBox::warning(this,"CIDR",tr("Эта запись содержит CIDR-ограничения из версии 1. Их нельзя редактировать в версии 2.","This entry contains legacy CIDR constraints and cannot be edited in version 2."));
		return;
	}
	auto m=readCertificate(QByteArray::fromBase64(state_.entries[r].certificateBase64.toLatin1()));
	QStringList d;
...
void MainWindow::validateState(const PolicyState& s) {
	for(const auto& e:s.entries) {
		if(e.certificateBase64.isEmpty())throw std::runtime_error("The backup contains an empty certificate.");
		auto m=readCertificate(QByteArray::fromBase64(e.certificateBase64.toLatin1()));
		if(!m.isCA)throw std::runtime_error("The selected certificate is not a CA certificate.");
...
void MainWindow::applyPolicies() {
	try {
		if(isolation_->isChecked())for(const auto& e:state_.entries) {
				if(!e.cidrs.isEmpty())throw std::runtime_error(tr("CIDR-ограничения из версии 1 нельзя применить в версии 2.","Legacy CIDR constraints cannot be applied in version 2.").toStdString());
				if(e.dnsNames.isEmpty())throw std::runtime_error(tr("Для каждого сертификата требуется хотя бы один домен.","Each certificate requires at least one domain.").toStdString());
			}
		...
		reload();
		QMessageBox::information(this,tr("Изменения применены","Changes applied"),tr("Изменения применены. Откройте страницу %1 для проверки.").arg(browser_.policyUrl));
		QApplication::clipboard()->setText(browser_.policyUrl);
	} catch(const std::exception& e) {
		QMessageBox::critical(this,tr("Ошибка","Error"),tr("Не удалось применить политики:\n\n","Could not apply policies:\n\n")+e.what());
	}
}
...
void MainWindow::exportPolicies() {
	QString p=QFileDialog::getSaveFileName(this,tr("Сохранить резервную копию","Save policy backup"),browser_.name+"CertificatePolicy-"+QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")+".json","Policy backups (*.json)");
	if(!p.isEmpty())try {
...
void MainWindow::importPolicies() {
	QString p=QFileDialog::getOpenFileName(this,tr("Выберите резервную копию","Select a policy backup"), {},"Policy backups (*.json);;All files (*)");
	if(p.isEmpty())return;
	try {
...
void MainWindow::closeEvent(QCloseEvent* e) {
	if(dirty()&&QMessageBox::question(this,tr("Есть неприменённые изменения","Unapplied changes"),tr("Закрыть программу и отбросить неприменённые изменения?","Close and discard unapplied changes?"))!=QMessageBox::Yes) {
		e->ignore();
		return;
	}
