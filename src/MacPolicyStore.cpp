...
PolicyState MacPolicyStore::load() {
	...
	while (!r.atEnd()) {
		r.readNext();
		if (r.isStartElement() && r.name() == QLatin1String("key")) {
		...
		if (r.isStartElement() && key == "CACertificatesWithConstraints" && r.name() == QLatin1String("array")) {
			while (!r.atEnd()) {
				r.readNext();
				if (r.isEndElement() && r.name() == QLatin1String("array")) break;
				if (r.isStartElement() && r.name() == QLatin1String("dict")) {
					CertificatePolicyEntry e;
					QString k;
					while (!r.atEnd()) {
						r.readNext();
						if (r.isEndElement() && r.name() == QLatin1String("dict")) break;
						if (r.isStartElement() && r.name() == QLatin1String("key")) {
							k = r.readElementText();
							continue;
						}
						if (r.isStartElement() && k == "certificate" && r.name() == QLatin1String("string")) {
							e.certificateBase64 = r.readElementText();
							k.clear();
						} else if (r.isStartElement() && k == "constraints" && r.name() == QLatin1String("dict")) {
							QString ck;
							while (!r.atEnd()) {
								r.readNext();
								if (r.isEndElement() && r.name() == QLatin1String("dict")) break;
								...
								if (r.isStartElement() && r.name() == QLatin1String("array") && (ck == "permitted_dns_names" || ck == "permitted_cidrs")) {
									QStringList* dst = (ck == "permitted_dns_names" ? &e.dnsNames : &e.cidrs);
									while (!r.atEnd()) {
										r.readNext();
										if (r.isEndElement() && r.name() == QLatin1String("array")) break;
										if (r.isStartElement() && r.name() == QLatin1String("string"))
											dst->append(r.readElementText());
									}
									ck.clear();
								}
							}
