...
int main(int argc,char**argv){
	...
	QApplication::setApplicationName("QChromeCertificates");
	QApplication::setApplicationVersion(QCC_VERSION);
	if (translator.load(QLocale(),"qchrocert","_",":/i18n"))
		app.installTranslator(&translator);

	QCommandLineParser p;
	p.setApplicationDescription(QApplication::translate("main","Cross-platform Chrome certificate policy manager"));
	QCommandLineOption self("self-test");
	QCommandLineOption reg("registry-path",QApplication::translate("main","Windows test registry path"),"path");
	p.addOption(self);
	p.addOption(reg);
	p.process(app);
	if(p.isSet(self)){
		try{
			...
			CertificatePolicyEntry ce{"AQID",{"gosuslugi.ru",".gosuslugi.ru"},{"10.1.1.0/24"}};
			PolicyState s;
			s.platformIntegrationEnabled=false;
			s.entries={ce};
			auto j=serializeEntries(s.entries);
			if(!j.contains("\"certificate\":\"AQID\""))
				return 1;
			qInfo("Self-test passed.");return 0;
		} catch(const std::exception& e){
			qCritical("Self-test failed: %s",e.what());
			return 1;
		}
	}
