...
static QString fullName(X509_NAME *name) {
	...
	QString r=QString::fromUtf8(p,n); BIO_free(b); return r;
}
CertificateMetadata readCertificate(const QByteArray &der) {
	...
	CertificateMetadata m; m.der=der;
	...
	if(!X509_digest(x,EVP_sha1(),md,&mdlen)) { X509_free(x); throw std::runtime_error("Cannot calculate SHA-1."); }
	QStringList hex; for(unsigned i=0;i<mdlen;i++) hex<<QString("%1").arg(md[i],2,16,QChar('0'));
	m.sha1=hex.join(":").toUpper();
	const ASN1_TIME *na=X509_get0_notAfter(x);
	struct tm t{}; if(ASN1_TIME_to_tm(na,&t)==1) {
	...
	X509_free(x); return m;
}
CertificateMetadata readCertificateFile(const QString &path) {
	QFile f(path); if(!f.open(QIODevice::ReadOnly)) throw std::runtime_error(("Cannot open certificate: "+path).toStdString());
	...
	unsigned char *out=nullptr; int n=i2d_X509(x,&out); QByteArray der(reinterpret_cast<char*>(out),n); OPENSSL_free(out); X509_free(x);
